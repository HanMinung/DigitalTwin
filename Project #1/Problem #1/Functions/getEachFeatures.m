function [output1, output2, output3] = getEachFeatures(inputData, outputPrefixes, className)
    ...
    [BPFO, BPFI, BSF] = faultFreq();
    faultFreqs = {BPFO, BPFI, BSF};

    % Process input data with timeFeatures function
    for dataIdx = 1:size(inputData, 2) 
        segment_data = inputData(:, dataIdx);
        segment_name = sprintf('%s_%d', outputPrefixes, dataIdx);
        segment_features1 = staticFeatures(segment_data, className);
        segment_features1.Properties.RowNames = {segment_name};
        output1 = [output1; segment_features1];
        
        segment_features2 = envelopeFeatures(segment_data, faultFreqs, className);
        segment_features2.Properties.RowNames = {segment_name};
        output2 = [output2; segment_features2];
        
        segment_features3 = waveletFeatures(segment_data, className);
